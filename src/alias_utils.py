...
def _pick_column(
    ...
    *,
    ...
    if exclude:
        exclude_lower = {str(item).lower() for item in exclude if item}
    for candidate in candidates:
        lowered = candidate.lower()
        if lowered in exclude_lower:
            continue
...
def _detect_columns(df: pd.DataFrame) -> _AliasColumns:
    ...
    source = _pick_column(df.columns, source_candidates)
    target = _pick_column(df.columns, target_candidates, exclude=[source] if source else None)
